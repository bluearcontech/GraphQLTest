	...
	componentWillReceiveProps(nextProps) {
		if (this.state.pizzaSizes.length === 0 && nextProps.data.pizzaSizes.length > 0) {
			...
			const toppings = selectedPizza.toppings.map(item => {
				return { defaultSelected: item.defaultSelected, name: item.topping.name, price: item.topping.price }
			})
			this.setState({
					...
					<li style={{ listStyle: 'none' }} key={index}>
						<label key={index}>
							<input
								...
								onChange={(e) => this.selectToppings(e, index)}
							/>
							{item.name}
						</label>
					</li>
					...
					<div className="col-sm-12">
						{loading === true ? (
							...
							/>
								...
								{selectedPizza && (
									<select name='test' value={selectedPizza.name} onChange={this.selectPizza}>
										{pizzaSizes.map((pizza, index) => (
											<option key={index} value={pizza.name}>{pizza.name}</option>
										))}
									</select>
								)}
								{selectedPizza && (
								...
								{error && (
									<div style={{ color: 'red' }} onClick={() => { this.setState({ error: false }) }}>You can select {selectedPizza.maxToppings} toppings</div>
								)}
								{checkBoxes}
